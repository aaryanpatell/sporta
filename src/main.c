...
void reigster_bitshuffle_filter();
void save_image_as_jpg(const char *filename, float *data, int width, int height);
char* decompress_hdf5_dataset(const char *filename, const char *dataset_path, int index);
void hdf5_read(const char* filename);

void register_bitshuffle_filter() {
...
void save_image_as_jpg(const char *filename, float *data, int width, int height) {
    ...
    // Write the image data, scaling the float values to integers between 0 and 255
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            int value = (int)(data[i * width + j] * 255);
...
char* decompress_hdf5_dataset(const char* filename, const char* dataset_path, int index) {
    ...
    // Open the dataset
    dataset_id = H5Dopen(file_id, dataset_path, H5P_DEFAULT);
    if (dataset_id < 0) {
    ...
    num_images = (int)dims[0]; // Assuming the first dimension represents the number of images

    if (index < num_images) {
        // Read the specified index
        hsize_t start[3] = { index, 0, 0 }; // assuming index is related to the first dimension
        hsize_t count[3] = { 1, dims[1], dims[2] }; // read one image of size dims[1] x dims[2]
        ...
        printf("Hyperslab selected successfully\n");
        data_out = (char*)malloc(dims[1] * dims[2] * sizeof(char));
        if (data_out == NULL) {
        ...
        hid_t mem_dataspace = H5Screate_simple(3, count, NULL); // Create the memory dataspace
        status = H5Dread(dataset_id, H5T_NATIVE_CHAR, mem_dataspace, dataspace_id, H5P_DEFAULT, data_out);
        if (status < 0) {
        ...
        H5Sclose(mem_dataspace); // Close the memory dataspace
    }
    else {
...
void hdf5_read(const char* filename){
    ...
    hid_t file_id, data_id, detector_id, sample_id, gonio_id, beam_id, distance_id, wavelength_id, detector_x, detector_y, beamX_id, beamY_id;
    file_id = H5Fopen(filename, H5F_ACC_RDONLY, H5P_DEFAULT);
...
unsigned char* loadImageFromMemory(unsigned char* image_data, int image_size, int *width, int *height, int *depth) {
    //unsigned char* output = stbi_load_from_memory(image_data, image_size, width, height, depth, 1);
    // unsigned char* output = image_data;
    // printf("AFTER");
    // if (output == NULL) {
...
// function to save the MSER data as an image
void savePGM(const char* filename, unsigned char* data, int width, int height) {
    ...
    printf("Writing PGM header to file.\n");

    // Write the PGM header to file
    printf("width = %d, height = %d\n", width, height);
    fprintf(fp, "P5\n%d %d\n255\n", width, height);

    fprintf(fp, "P5\n%d %d\n255\n", width, height);    fprintf(fp, "P5\n%d %d\n255\n", width, height);

    fwrite(data, width*height*3, sizeof(unsigned char), fp);
    ...
    if (dark_on_bright) {
        ...
        */
        ...
        for (i = 0; i < width * height * depth; i++) {
            //datainv[i] = ~(data[i]); ///problem is HERE
        }
...
int main() {
    const char *filename = "/home/patela4/Documents/database/imager/data/1/CLS-0025_7-1_data_000001.h5";
    const char *dataset_path = "/entry/data/data";
    int index = 1;
    hdf5_read("/home/patela4/Documents/database/imager/data/1/CLS-0025_7_master.h5");
    char *data = decompress_hdf5_dataset(filename, dataset_path, index);
    if (data != NULL) {
        //save_image_as_jpg("donut.jpg", data, 3110, 3269);
        int width = 3110;
        int height = 3269;
        int image_size = width * height;
        int depth = 1;
        float delta = 50;
        float max_area = 0.00002f;
        float min_area = 0.000001f;
        float max_variation = 1000.0f;
        float min_diversity = 0.0001f;
        int dark_on_bright = 0;
        //unsigned char *data = loadImage("CLS-0025_7-1_data_000007.jpg", &width, &height, &depth);
        unsigned char *data = loadImageFromMemory(data, image_size, &width, &height, &depth);
        CPUImageMser(data, width, height, depth, delta, max_area, min_area, max_variation, min_diversity, dark_on_bright);
        printf("%c", data[0]);
        //savePGM("output.pgm", data, width, height);
        //free(data);
    } else {
